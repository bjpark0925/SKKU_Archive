...
//경로 존재 여부
int path_exist(graphType* g, char vertex[], char start, char destination)
{
	int len = g->vertexCount;
...
int is_completeGraph(graphType* g)
{
	...
	for (int i = 0; i < len; i++) {
		int cnt = 0;
		for (int j = 0; j < len; j++) { // 한 행에서 자기 자신을 제외한 열은 0이 아니어야 완전 그래프임.
			if (g->matrix[i][j] == 0)
				cnt++;
		}
		if (cnt > 1) { // 완전 그래프가 아닌 경우
			return -1; // false
		}
	}
...
int path_weight(graphType* g, char vertex[], char start, char destination)
{
	int exist = path_exist(g, vertex, start, destination);
...
void UserSetting()
{
	...
	printf("U : 무방향 그래프 여부\n");
	printf("W(AB) : 경로의 가중치 출력\n");
	printf("!: vertex배열 출력(확인용)\n");
...
int main(void) {
	...
	while (1)
	{
		...
		if (order[0] == 'P' && order[1] == '(' && order[strlen(order) - 1] == ')' && strlen(order) == 5) { // P(AB)로 입력받은 경우
			if (path_exist(graph, vertex, order[2], order[3]) == 1)
				printf("True\n");
			else
				printf("False\n");
		}
		...
		if (order[0] == 'W' && order[1] == '(' && order[strlen(order) - 1] == ')' && strlen(order) == 5) { // W(AB)로 입력받은 경우
			if (path_weight(graph, vertex, order[2], order[3]) == -1)
				printf("Error\n");
			else
				printf("%d\n", path_weight(graph, vertex, order[2], order[3]));
		}
	}
