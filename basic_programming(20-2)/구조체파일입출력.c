...
void writeAllToFile()
{
	...
	fprintf(fout, "\n\n");
	fprintf(fout, "============================================================\n");
	fprintf(fout, "%13s | %10s | %10s | %6s\n", "School Number", "Department", "Name", "Grade");

	for (int i = 0; i < studentsCount; i++) {
		fprintf(fout, "------------------------------------------------------------\n");
		fprintf(fout, "%13d | %10s | %10s | %6.2lf\n",
			students[i].nSchoolNumber,
			students[i].strDepartment,
			students[i].strName,
			students[i].lfGrade);
...
void readFromFile()
{
	...
	while (!feof(fin)) {
		fscanf(fin, "%d", &students[studentsCount].nSchoolNumber);
		fscanf(fin, "%s", students[studentsCount].strDepartment);
		fscanf(fin, "%s", students[studentsCount].strName);
		fscanf(fin, "%lf", &students[studentsCount++].lfGrade);
	}//while
