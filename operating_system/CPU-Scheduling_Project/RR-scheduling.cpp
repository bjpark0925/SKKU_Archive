...
#define NUM_CHILD 10
#define TIME_QUANTUM 100 // mili-second
...
void printData(analyzeData &data)
{
    ...
    if (data.count != 0)
    {
        ...
        printf("average turnaround time: %dms\n", data.turnaround_time / data.count);
        printf("total number of process terminated: %d\n", data.count);
    }
    else
    {
        printf("there is no terminated process\n");
    }
}
...
void exitProgram(int sig)
{
    ...
    if (is_sigint)
    {
        ...
        printf("ready queue clear complete\n");
        // io queue clear
        ...
        printf("io queue clear complete\n");
    }
    ...
    {
        printf("[msqid %d] msgctl success!\n", msqid);
    }

    printf("===============Information================\n");
    printf("running time: %llds\n", (end - start) / 1000);
...
void schedule(int sig)
{
    ...
    if (!ready_queue.empty()) // ready queue에 process가 있으면
    {
        ...
        {
            printf("parent's msgsnd failed1\n");
            exit(1);
        }
    }
    ...
    if (!io_queue.empty()) // io queue에 process가 있으면
    {
        ...
        if (msgsnd(msqid, &msg, sizeof(process), 0) == -1)
        {
            printf("parent's msgsnd failed2\n");
            exit(1);
        }
    }
    ...
    printf("Running: [pid: %d, quantum: %d, cpu burst: %d, io burst: %d, remainIO: %d, io start: %d, is_io: %d, state: %d, arrival: %d, response: %d, wait: %d]\n",
    ...
    {
        printf("[PID: %d, CPU Burst: %d]", temp_queue.front().pid, temp_queue.front().cpu_burst);
        temp_queue.pop();
    }
    printf("\n");
    ...
    while (!temp_queue.empty())
    {
        printf("[PID: %d, IO Burst: %d, CPU Burst: %d]", temp_queue.front().pid, temp_queue.front().remainIO, temp_queue.front().cpu_burst);
        temp_queue.pop();
    }
    printf("\n");
